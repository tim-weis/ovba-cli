...
impl Document {
    ...
    pub(crate) fn new(source: &Option<PathBuf>) -> Result<Self, ovba::error::Error> {
        match source {
            Some(path_name) => Ok(Self {
                data: read(path_name).map_err(|e| ovba::error::Error::Io(e.into()))?,
            }),
            None => {
    ...
    pub(crate) fn vba_project_name(&self) -> Result<Option<String>, ovba::error::Error> {
        let factory = Factory::new();
        let xpath = factory
            .build(
                "/ns:Types/ns:Override[@ContentType='application/vnd.ms-office.vbaProject']/@PartName",
            )
            .map_err(|e| ovba::error::Error::InvalidDocument(e.into()))?
            .unwrap();
    ...
    pub(crate) fn part(&self, part_name: &str) -> Result<Vec<u8>, ovba::error::Error> {
    ...
    fn content_types(&self) -> Result<String, ovba::error::Error> {
